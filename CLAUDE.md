...
- **`shared/version.ts` is the single source of truth for the version** (served at `/api/version`, shown in sidebar + login) and the *only* place to edit it — bumping it on `main` is what makes CI cut the release, see Deployment. The README badge reads the latest GitHub release from shields.io rather than hardcoding a number, so there is nothing to keep in sync.
...
**Never commit to `main` directly — branch, open a PR, merge it.** `main` is protected, and `.github/workflows/pr-checks.yml` gates every PR: it runs `npm test` and `npm run typecheck` (nothing else in CI does — the image build doesn't), and it fails the PR when app code changed without a `shared/version.ts` bump *and* a matching `## X.Y.Z` section in `CHANGELOG.md`. Changes confined to `.github/`, `docs/` or the `*.md` files are exempt, because forcing a release for those would mean a version per PR.
