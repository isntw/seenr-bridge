...
export function deleteMapping(id: number): void {
  ...
  db.prepare('DELETE FROM shared_titles WHERE rating_key NOT IN (SELECT rating_key FROM shared_title_profiles)').run();
...
export function listSharedTitles(): SharedTitle[] {
  const rows = db.prepare('SELECT * FROM shared_titles ORDER BY title').all() as Omit<SharedTitle, 'profiles'>[];
  const links = db.prepare('SELECT rating_key, mapping_id FROM shared_title_profiles').all() as { rating_key: string; mapping_id: number }[];
  ...
  return rows.map((r) => ({ ...r, profiles: byKey.get(r.rating_key) || [] }));
}
