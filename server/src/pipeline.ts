import { getSettings, getMappingByUsername, getSharedRecipients, listSharedTitles, insertEvent, type Mapping, type Settings } from './db';
import { getMetadata, getWatchedEpisodeKeys, type TautulliMetadata } from './tautulli';
  ...
  try {
    ...
    if (record)
      insertEvent({
        ...
        seenr_status: null, ok: 0, error: `Forward to seenr failed: ${e?.message || e}`, payload: JSON.stringify(built.payload),
      });
    return { ok: false };
  }
...
export async function processEvent(input: IncomingEvent, opts: { dryRun?: boolean; record?: boolean } = {}): Promise<ProcessResult> {
  ...
  let recipients: Mapping[] = [trigger];
...
export async function backfillSharedTitle(ratingKey: string): Promise<BackfillResult> {
  ...
  if (share.media_type === 'movie') {
    ...
    try {
      ...
      return { ok: false, reason: `History lookup failed: ${e?.message || e}`, ...empty };
    }
    if (!itemKeys.length) return { ok: false, reason: 'No watched episodes found in Tautulli history', items: 0, profiles: profiles.length, delivered: 0, ok_count: 0, fail_count: 0 };
  }
