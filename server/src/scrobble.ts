import { TautulliMetadata } from './tautulli';

// Map a Tautulli notify action to the Plex webhook event name that seenr expects.
...
// Build a Plex `media.scrobble`-style payload using the item's OWN external IDs.
// For episodes, m.guids are the episode's real ids (tmdb/tvdb/imdb of the episode),
// which is exactly what the Tautulli webhook template could not provide.
export function buildPayload(m: TautulliMetadata, action: string, username: string): BuiltPayload {
